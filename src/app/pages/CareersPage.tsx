...
function ApplyModal({ job, onClose }: { job: typeof openings[0]; onClose: () => void }) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const subject = encodeURIComponent(`Job Application: ${job.title} — ${form.name}`);
    const body = encodeURIComponent(
      `Position: ${job.title}\nStudio: ${job.studio}\n\nApplicant Name: ${form.name}\nEmail: ${form.email}\nPhone: ${form.phone}\n\nCover Letter:\n${form.cover || "N/A"}\n\n${cvFile ? `CV File: ${cvFile.name} (please attach manually)` : "No CV attached."}`
    );
    window.location.href = `mailto:careers@amisgroups.com?subject=${subject}&body=${body}`;
