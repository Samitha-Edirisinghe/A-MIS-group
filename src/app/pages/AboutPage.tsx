...
import { ArrowUpRight, Target, Eye, Heart, Zap, Globe } from "lucide-react";
...
const team = [
  { name: "A'MIS Creotech Team", role: "Technology & Creative", image: "https://images.unsplash.com/photo-1758873268631-fa944fc5cad2?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=600" },
  { name: "A'MIS Captura Team",  role: "Photography Studio",    image: "https://images.unsplash.com/photo-1621024994278-e409544f4085?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=600" },
  { name: "A'MIS Cinemix Team",  role: "Video Production",      image: "https://images.unsplash.com/photo-1632187981988-40f3cbaeef5e?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=600" },
];
...
export function AboutPage() {
  return (
    <div className="min-h-screen bg-white">
      ...
      {/* ── HERO ── */}
      <section className="relative min-h-screen overflow-hidden pt-16 flex flex-col">
        {/* Full-bleed background image */}
        <div className="absolute inset-0">
          <img
            src="https://images.unsplash.com/photo-1758873268663-5a362616b5a7?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1600"
            alt="A'MIS Group team"
            className="w-full h-full object-cover"
          />
          <div className="absolute inset-0 bg-gradient-to-r from-[#0b1940]/95 via-[#0b1940]/70 to-[#0b1940]/30" />
          <div className="absolute bottom-0 left-0 right-0 h-40 bg-gradient-to-t from-[#0b1940] to-transparent" />
        </div>
        ...
        <div className="relative z-10 flex-1 flex flex-col justify-center max-w-7xl mx-auto px-6 lg:px-16 py-16 md:py-28 w-full">
          <motion.div initial={{ opacity: 0, y: 36 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: [0.22, 1, 0.36, 1] }} className="max-w-2xl">
            <div className="flex items-center gap-3 mb-6 md:mb-8">
              ...
              <span className="text-xs font-bold tracking-[0.28em] text-[#c8f135] uppercase">About A'MIS Group</span>
            </div>
            ...
            <div className="flex flex-wrap gap-3">
              <a href="#/services" className="inline-flex items-center gap-2 px-6 py-3 md:px-8 md:py-4 bg-[#c8f135] text-[#0b1940] font-black text-sm rounded hover:bg-[#d5f84a] transition-colors">
                Our Services <ArrowUpRight className="w-4 h-4" />
              </a>
              <a href="#/contact" className="inline-flex items-center gap-2 px-6 py-3 md:px-8 md:py-4 border border-white/25 text-white font-bold text-sm rounded hover:border-white/60 transition-colors">
                Get In Touch
              </a>
            </div>
          </motion.div>
        ...
        <div className="relative z-10 border-t border-white/10">
          <div className="max-w-7xl mx-auto px-6 lg:px-16">
            <div className="grid grid-cols-2 md:grid-cols-4">
              {stats.map((s, i) => (
                <div key={s.label}
                  className={`group py-5 md:py-7 px-4 md:px-8 flex items-center gap-4 md:gap-5 relative transition-colors duration-200 hover:bg-white/[0.03] ${
                    i < 3 ? "after:absolute after:right-0 after:top-5 after:bottom-5 after:w-px after:bg-white/8" : ""
                  }`}>
                  <div className="w-1 h-8 md:h-10 rounded-full bg-[#c8f135]/40 group-hover:bg-[#c8f135] transition-colors duration-300 shrink-0" />
                  <div>
                    <div className="text-2xl md:text-3xl font-black text-white leading-none tracking-tight group-hover:text-[#c8f135] transition-colors duration-300">
                      {s.value}
                    </div>
                    <div className="text-white/35 text-[10px] md:text-[11px] font-semibold tracking-[0.14em] uppercase mt-1.5">{s.label}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="grid lg:grid-cols-2 gap-10 lg:gap-16 items-center">
            ...
            <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.1 }} className="relative">
              <div className="rounded-2xl overflow-hidden">
                <img src="https://images.unsplash.com/photo-1690378820474-b468b8ee64d3?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080" alt="A'MIS team" className="w-full h-64 md:h-[400px] lg:h-[500px] object-cover" />
              </div>
